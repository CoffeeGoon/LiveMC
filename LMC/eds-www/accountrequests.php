...
$countB = 0;
while($ret != 0){
	$jsarray =  $jsarray . "'" . $ret['name'] . "',";
	$countA = $countA + 1;
	$ret = $data->fetch_assoc();
}
If($tp == "venue"){
$data = $msqli->query("SELECT name, band_bio FROM band_account where band_account.active != 'false' ");
$ret = $data->fetch_assoc();
while($ret != 0){
	 $add = " [ '" . $ret['name'] . "', '" . $ret['band_bio'] . "'],";
	$avail = $avail . $add;
	$countB = $countB + 1;
	$ret = $data->fetch_assoc();
}
...
$data = $msqli->query("SELECT name, active, venue_bio FROM venue_account where venue_account.active != 'false' ");
	$ret = $data->fetch_assoc();
while($ret != 0){
	$add = " [ '" . $ret['name'] . "', '" . $ret['active'] . "' , '" . $ret['venue_bio'] . "'],";
	$avail = $avail . $add;
	$countB = $countB + 1;
	$ret = $data->fetch_assoc();
}
